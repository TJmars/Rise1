...
class FourthViewController: UIViewController, IndicatorInfoProvider {
    ...
    //必須
    var itemInfo: IndicatorInfo = "総時間"
...
//    グラフのデータ用変数
    ...
    override func viewDidLoad() {
        ...
        self.leftAxisLabel.frame = CGRect(x: 0, y: 320, width: 30, height: 10)
        self.leftAxisLabel.font = UIFont(name:"DINAlternate-Bold", size: 11.0)!
        self.rightAxisLabel.frame = CGRect(x: chartView.frame.size.width - 30, y: 320, width: 30, height: 10)
        self.rightAxisLabel.font = UIFont(name:"DINAlternate-Bold", size: 11.0)!
        self.leftAxisLabel.text = "(個/h)"
        self.rightAxisLabel.text = "(秒)"
...
//            グラフの数値を作成
            ...
            for i in 0...8 {
                ...
                for i2 in 0..<genreNameList.count {
                    let taskArray = try! Realm().objects(TimeData.self).filter("date >= %@ && date <= %@ && genre == %@ ", startDay, endDay, genreNameList[i2])
                ...
                graphDataArray.append(totalTime)
                doneTaskArray.append(lineDataNum)
            }
            let num = graphDataArray.max()
            let lineTotalTimeArray = graphDataArray
            if num! >= 18000 {
//                    時間に直す
                for i4 in 0..<graphDataArray.count {
                    if graphDataArray[i4] != 0 {
                        graphDataArray[i4] = graphDataArray[i4] / 3600
                    }
                }
                self.rightAxisLabel.text = "(時間)"
            } else if num! >= 300 {
//                    分に直す
                for i4 in 0..<graphDataArray.count {
                    if graphDataArray[i4] != 0 {
                        graphDataArray[i4] = graphDataArray[i4] / 60
                    }
                }
                self.rightAxisLabel.text = "(分)"
            }
            
            for i5 in 0..<lineTotalTimeArray.count {
                if doneTaskArray[i5] != 0 {
                    let num = lineTotalTimeArray[i5] / 3600
                    lineGraphDataArray.append(doneTaskArray[i5] / num)
                } else {
                    lineGraphDataArray.append(0)
                }
            }
...
//            グラフの数値を作成
            for i in 0...8 {
                ...
                for i2 in 0..<genreNameList.count {
                    let taskArray = try! Realm().objects(TimeData.self).filter("date >= %@ && date <= %@ && genre == %@ ", startMonth, lastMonth, genreNameList[i2])
                    
                    if  taskArray.count != 0 {
                        totalTime += Double(taskArray[0].totalTime)
                    }
                    
                    let lineTaskArray = try! Realm().objects(Task.self).filter("date >= %@ && date <= %@ && genre == %@ && inTimeCheck == 2 ", startMonth, lastMonth, genreNameList[i2])
                    lineDataNum += Double(lineTaskArray.count)
                }
                graphDataArray.append(totalTime)
                doneTaskArray.append(lineDataNum)
...
//                    分に直す
        ...
        let lineDataSet = LineChartDataSet(entries: lineData, label: "時間効率")
        ...
        let barDataSet = BarChartDataSet(entries: barData, label: "総時間")
